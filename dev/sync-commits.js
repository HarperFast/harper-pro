 ...
 * old HarperDB repository to the new Harper Pro repository. - WSM 2026-02-18
 */

const originBranch = process.env['ORIGIN_BRANCH'] || 'main';
const oldBranch = process.env['OLD_BRANCH'] || 'main';
const useCurrentBranch = process.env['USE_CURRENT_BRANCH'] === 'true';

function letsBail(exitCode, syncBranch = null) {
	execSync(`git checkout ${originBranch}`, { stdio: 'ignore' });
	if (syncBranch) {
...
function isOriginRemoteConfigured() {
	return verifyRemote('origin', 'git@github.com:HarperFast/harper-pro.git');
}
...
function pullRemoteBranch(remoteName, branchName) {
	...
	exec(`git merge ${remoteName}/${branchName}`, (error, _stdout, stderr) => {
		if (error) {
			console.error(`git exited with error '${error.message}' merging origin/${originBranch}`);
			letsBail(error.code);
		}
		if (stderr) {
			console.error(`git error merging origin/${originBranch}: ${stderr}`);
			letsBail(6);
		}
	});
}
...
function ensureValidConfig() {
	...
	if (!isOriginRemoteConfigured()) {
		...
		console.error(
			'Run `git remote add origin git@github.com:HarperFast/harper-pro.git` to configure it (you may have to remove the origin remote first with `git remote rm origin`).'
		);
		process.exit(3);
	}
	if (!isBranchCheckedOut(originBranch)) {
		console.log('❌');
		console.error(`${originBranch} branch not checked out. Run \`git checkout ${originBranch}\` to check it out.`);
		process.exit(4);
	}
	console.log('✅');
}
...
function generateCommitsToPick(startCommit) {
	const commits = execSync(`git rev-list --reverse --first-parent ${startCommit}..old/${oldBranch}`)
		.toString()
		.trim()
		.split('\n');

...
function auditRecords(records, originRecordsById) {
	...
	for (const rec of records) {
		...
		if (!origin) continue;
		const missing = [];
		if (origin.f1 !== undefined && rec.f1 !== origin.f1)
			missing.push(`f1: got ${JSON.stringify(rec.f1)}, want ${JSON.stringify(origin.f1)}`);
		if (origin.f2 !== undefined && rec.f2 !== origin.f2)
			missing.push(`f2: got ${JSON.stringify(rec.f2)}, want ${JSON.stringify(origin.f2)}`);
		if (origin.f3 !== undefined && rec.f3 !== origin.f3)
			missing.push(`f3: got ${JSON.stringify(rec.f3)}, want ${JSON.stringify(origin.f3)}`);
		if (origin.f4 !== undefined && rec.f4 !== origin.f4)
			missing.push(`f4: got ${JSON.stringify(rec.f4)}, want ${JSON.stringify(origin.f4)}`);
		if (missing.length) {
...
suite(
	'Typed-struct replication divergence resistance (randomAccessFields:true)',
	...
	(ctx) => {
		...
		after(async () => {
			await Promise.all(Object.values(ctx.nodes).map((node) => teardownHarper({ harper: node }).catch(() => null)));
		});

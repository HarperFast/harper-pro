...
suite(
	'Replication conflict-resolution determinism (2-node bidirectional)',
	{ skip: !STRESS, timeout: 600000 },
	(ctx) => {
		before(async () => {
		...
		test(`plain-field concurrent writes — ${RACE_TRIALS} LWW trials`, async (t) => {
			...
			for (let i = 0; i < RACE_TRIALS; i++) {
				...
				const [finalA, finalB] = await Promise.all([getRecord(nodeA, id), getRecord(nodeB, id)]);
				...
				const torn =
					(finalA?.score === scoreA && finalA?.label === labelB) ||
			...
			equal(
				...
				`LWW CONVERGENCE FAILURE: ${results.diverged} trials ended with A and B holding different values.\n` +
					JSON.stringify(
						results.perTrial.filter((t) => !t.converged),
						null,
						2
					)
			);

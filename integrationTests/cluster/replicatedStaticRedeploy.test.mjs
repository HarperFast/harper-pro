...
suite('replicated redeploy of a static-file component (gh#1935 regression anchor)', { timeout: 300000 }, (ctx) => {
	...
	test('initial deploy: control + existing pages serve on both nodes', { timeout: 90_000 }, async () => {
	...
	test('restart:true on the replica only fixes that node; origin unaffected', { timeout: 90_000 }, async () => {
		const before = ctx.snapshots;
		console.log('[QA-710] pre-restart snapshots:', JSON.stringify(before));

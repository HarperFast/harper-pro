...
14. **A replicated transaction has one origin, and a frame is not a transaction.** The unique identity of a transaction-log entry is (`txnLogKey`, `nodeId`); the key alone repeats across origins. A sender frame holds every entry at one key across the sender's logs (every subscriber sends an exclusion list, so the merged all-logs walk is the normal send path), so it can carry several origins' transactions. Two receive-side rules keep core's one-transaction-at-a-time apply loop on one origin: (a) within a frame, a record whose origin differs from the previous record's opens a new transaction (`beginTxn`), because RocksDB binds a transaction to one origin's log. Copy-apply rows are exempt: they write no log entry, so they neither open a transaction nor become the origin the next audited record is compared with (base-copy frames list rows in key order, so splitting them made roughly one transaction per row). The resume cursor still advances only at the frame's `end_txn`, after every segment commits, and a crash between segments replays the whole frame, whose already-applied records core drops as version-and-origin ties. Splitting on the sender instead would put two frames at one key, and the `exclusiveStart` resume would skip the second after a crash between them. (b) Every connection on a thread feeds one subscription per database, so a connection holds `takeFrameTurn` from its first queued event until the `end_txn` that closes it, and standalone `end_txn` events (`SEQUENCE_ID_UPDATE`, blob-drain re-emit) take the turn too; otherwise another peer's events land inside the open transaction, mixing and splitting both (harper#1162). A connection waiting for the turn pauses its socket, for the same retained-frame reason as the placeholder-subscription item above.

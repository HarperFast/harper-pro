...
export function redactOperationForLog(operation: any): any {
	...
	// credentials entries are normally references ({ registry, secret, scope? }) with no token —
	...
	if (Array.isArray(operation.credentials)) {
		...
		const maskedAuth = operation.credentials.map((entry: any) => {
		...
		if (redactedAuth) ensureCopy().credentials = maskedAuth;
	}

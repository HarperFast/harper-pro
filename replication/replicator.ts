...
export function start(options) {
	...
	logger.notify('replication server hostname', server.hostname);
	if (!server.hostname) throw new Error('Can not load replication without a url (see replication.url in the config)');
		...
		if (request.isWebSocket && request.headers.get('Sec-WebSocket-Protocol') === 'harperdb-replication-v1') {
			...
			if (authorizationError) {
				if (authorizationError === 'CERT_HAS_EXPIRED') {
					logger.error(
						`Incoming client connection from ${request.peerCertificate?.subjectaltname ?? request.ip} has an expired certificate`,
						authorizationError
					);
				} else {
					logger.error(

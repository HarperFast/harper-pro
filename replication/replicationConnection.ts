...
export function replicateOverWS(ws: WebSocket, options: any, authorization: any) {
	...
	copyProgressWatchdog = createReceiveWatchdog({
		...
		onSilence: () => {
			...
			const dbContext = databaseName ? ` (db: "${databaseName}")` : '';
			logger.warn?.(
				`Copy-progress watchdog: no base-copy progress from ${remoteNodeName}${dbContext} for ${effectiveBlobTimeoutMs}ms (and through a ${Math.max(PING_INTERVAL * 2, 1000)}ms confirmation) while peer bytes kept arriving — terminating connection and reconnecting to restart the copy (harper-pro#453) — ${fireTelemetryForLog('copy-progress')}`
			);
			if (options.connection) options.connection.forceReconnect();
			else ws.terminate();
		},

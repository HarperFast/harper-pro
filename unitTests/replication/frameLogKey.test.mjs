/**
 * harper-pro#790: a replication frame's leading float64 is the origin's log key for every record in
...
import { isValidFrameLogKey } from '#src/replication/replicationConnection';

describe('replication frame log key validation', () => {
	it('accepts a representable date', () => {
		expect(isValidFrameLogKey(Date.now())).to.equal(true);
		expect(isValidFrameLogKey(1)).to.equal(true);
		// the audit clock is sub-millisecond on RocksDB, so a fractional key is ordinary
		expect(isValidFrameLogKey(Date.now() + 0.0269)).to.equal(true);
		expect(isValidFrameLogKey(8.64e15)).to.equal(true);
	});

	it('rejects values that cannot be a log position', () => {
		expect(isValidFrameLogKey(NaN)).to.equal(false);
		expect(isValidFrameLogKey(Infinity)).to.equal(false);
		expect(isValidFrameLogKey(-Infinity)).to.equal(false);
		expect(isValidFrameLogKey(0)).to.equal(false);
		expect(isValidFrameLogKey(-1)).to.equal(false);
		...
		expect(isValidFrameLogKey(8.64e15 + 1)).to.equal(false);
	});

	it('rejects a non-number, so a malformed frame cannot masquerade as a key', () => {
		expect(isValidFrameLogKey(undefined)).to.equal(false);
		expect(isValidFrameLogKey(null)).to.equal(false);
		expect(isValidFrameLogKey('1788469796313')).to.equal(false);
	});

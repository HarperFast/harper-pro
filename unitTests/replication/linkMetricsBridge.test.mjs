 ...
 *
 * The asymmetry these tests pin: `latency` is only copied when the buffer carries one AND truth reports the
 * link connected — 0 means "no pong yet" and copying it would replace the value the connect edge already
 * mirrored with a false instant reading, and on a down link a nonzero reading belongs to some other
 * connection sharing the (database, peer) key. `backPressureRatio` is copied unconditionally, because 0 is
 * its meaningful "no back-pressure" value.
 */
...
describe('copyLinkMetricsToEntry', () => {
	it('copies both signals onto the entry', () => {
		const entry = {};
		copyLinkMetricsToEntry(entry, makeStatus({ latency: 12.5, backPressure: 0.25 }), true);
	...
	it('keeps the entry latency the connect edge mirrored when the buffer has no pong reading yet', () => {
		...
		copyLinkMetricsToEntry(entry, makeStatus({ latency: 0, backPressure: 0.5 }), true);
		expect(entry.latency).to.equal(8);
		expect(entry.backPressureRatio).to.equal(0.5);
	});

	it("leaves latency alone on a down link, where a fresh reading is another connection's", () => {
		const entry = { latency: 8 };
		copyLinkMetricsToEntry(entry, makeStatus({ latency: 12.5, backPressure: 0.25 }), false);
		expect(entry.latency).to.equal(8);
		expect(entry.backPressureRatio).to.equal(0.25);
	});
	...
	it('copies a zero back-pressure ratio, since zero is the meaningful "no back-pressure" reading', () => {
		...
		copyLinkMetricsToEntry(entry, makeStatus({ backPressure: 0 }), true);
	...
	it('agrees with the backPressurePercent view clusterStatus reports from the same slot', () => {
		...
		copyLinkMetricsToEntry(entry, status, true);
	...
	it('reads only its own two slots, so nothing else on the buffer is disturbed', () => {
		...
		copyLinkMetricsToEntry({}, status, true);

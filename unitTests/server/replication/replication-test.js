...
const { servers, sendOperationToNode } = require('../../../dist/replication/replicator');
const { databases, resetDatabases } = require('../../../dist/core/resources/databases');
...
const { ResourceBridge } = require('../../../dist/core/dataLayer/harperBridge/ResourceBridge');
const { open } = require('lmdb');
const { transaction } = require('../../../dist/core/resources/transaction');
const readLog = require('../../../dist/core/utility/logging/readLog');
const { AUDIT_STORE_OPTIONS, readAuditEntry } = require('../../../dist/core/resources/auditStore');
const { OpenDBIObject } = require('../../../dist/core/utility/lmdb/OpenDBIObject');
...
describe('Replication', () => {
	...
	before(async function () {
		...
		database_config = env_get('databases');
	...
	it('read_log should be replicated', async function () {
		...
		assert(nodes.has('node-1') || nodes.has('127.0.0.1'));
...
function createTestDBPath() {
	let dbPath = join(__dirname, 'envDir', process.pid.toString());
	env.setProperty('HDB_ROOT', dbPath);
	env.setProperty('databases', { data: { path: dbPath }, dev: { path: dbPath } });
	env.setProperty('storage_path', dbPath);
	resetDatabases();
	return dbPath;
}
